# -*- coding: utf-8 -*-
"""Real or Fake news Classifications.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Jlblm5zrpQ4lED0EZjA8p4aL5ebMn2r1

# **Real or Fake news Classifications**
...
df = df.rename(columns={'text': 'Content', 'label' : 'Labels'})
df
...
negative_data = df[df["Labels"] == "REAL"]
positive_data = df[df["Labels"] == "FAKE"]
...
df['Labels'].value_counts().plot(kind='bar', color=['green','red'])
...
#content ganti ke steming_2
pos = df.loc[df['Labels'] == 'REAL'].Content.tolist()
neg = df.loc[df['Labels'] == 'FAKE'].Content.tolist()

pos_label = df.loc[df['Labels'] == 'REAL'].Labels.tolist()
neg_label = df.loc[df['Labels'] == 'FAKE'].Labels.tolist()

total_data = pos + neg
labels = pos_label + neg_label

print("REAL: %s \nFAKE: %s" % (len(pos),len(neg)))
...
class myCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs={}):
        if (logs.get('accuracy') > 0.81 and logs.get('val_accuracy') > 0.80):
